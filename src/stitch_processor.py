...
#
    ...
    def process(self, manual_crop_list=None, threshold_multiplier = -3.0, window = 60, binsize = 0.05, clear_storage = True):
        ...
        for i in range(len(tdt_data_list)):
            ...
            self.data['clean_neural'].append(clean_filtered_neural)
    ...
    def phase_train(self, upper_limit = 2, lower_limit = 0.7, angles = None, phase = None, rates = None, forcible_test_phase = None, forcible_test_rates = None):
        #ALERT: modify these steps if the CNN has been trained to identify 0 already
        #this loop is used to manually drop frames from a 0&1 CNN by assigning undesired frames a value of (-1)
        ...
        angles_raw = form_angles[np.nonzero(preform_phase),:][0]
        rates_raw = form_rates[np.nonzero(preform_phase),:][0]
        metric = self.angle_name_helper('limbfoot')
        h_angle, vaf_array, test_angle_rates, test_angle_angle, _ = decode_kfolds(rates_12,angles_12,metric_angle=metric, vaf_scoring = False)
...
#         return np.mean((r2_sin_nl,r2_cos_nl), axis = 0), h_sin_nl, lsq_sin, h_cos_nl, lsq_cos, predicted_arctans_nl, test_arctans, swing_mean
        #########################
    
        sin_arr, cos_arr = sine_and_cosine(phase_angles)    
        score, h_sin, h_cos, test_rates, test_arctans, predicted_arctans, _ = parallel_decoder(X=rates_12, Y1=sin_arr, Y2=cos_arr, printing = True)
        
        if (forcible_test_rates is not None) and (forcible_test_phase is not None):
            predicted_sin = test_wiener_filter(forcible_test_rates, h_sin)
            predicted_cos = test_wiener_filter(forcible_test_rates, h_cos)
            predicted_arctans = arctan_fn(predicted_sin, predicted_cos)
            test_arctans = forcible_test_phase
            sin_arr, cos_arr = sine_and_cosine(forcible_test_phase)        
        ...
        return score, h_sin, h_cos, predicted_arctans, test_arctans, test_rates, swing_mean, vaf_array, h_angle, test_angle_rates, test_angle_angle
